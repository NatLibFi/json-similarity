...
function factory(Object, objectCompare, jsonpath)
{
...
Number.isNaN = Number.isNaN || function(value) {     
  ...
  function trim(chars, value, side)
  {
    ...
    if (typeof value === 'string') {
      value = replaceChars(chars, value, side);
    } else if (typeof value === 'number') {
      value = Number(replaceChars(chars, value.toString(), side));
    }
    ...
    function getAverage(subject_a, subject_b)
    {
      ...
      var matching = subject_a.reduce(function(match_count, result_a, index) {    
        ...
        if (subject_b.some(function(result_b) {
          return objectCompare(applyModifications(result_a.value, options), applyModifications(result_b.value, options), options);
        })) {
      ...
      match: !spec.tests.some(function(test, index_test) {
        ...
        if (test.disabled === true) {
          ...
          if (!((result1.length === 0 && result2.length === 0) || (skip_missing && (result1.length === 0 || result2.length === 0)))) {
            ...
            if (result1.length > 1 && result2.length > 1 || options.multimatch.force === true && (result1.length > 1 || result2.length > 1)) {
